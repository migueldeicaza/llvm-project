...
//===----------------------------------------------------------------------===//
//
// This file provides a library for running a module on a Metal device.
// Implements a Metal runtime.
//
//===----------------------------------------------------------------------===//
...
void MetalRuntime::setNumWorkGroups(const NumWorkGroups &numberWorkGroups) {
  numWorkGroups = numberWorkGroups;
}

void MetalRuntime::setResourceStorageClassBindingMap(
    const ResourceStorageClassBindingMap &stClassData) {
  resourceStorageClassData = stClassData;
}

void MetalRuntime::setResourceData(
...
void MetalRuntime::setEntryPoint(const char *entryPointName) {
  entryPoint = entryPointName;
}

void MetalRuntime::setResourceData(const ResourceData &resData) {
  resourceData = resData;
}

void MetalRuntime::setShaderModule(uint8_t *shader, uint32_t size) {
  binary = shader;
  binarySize = size;
}

#if 0
LogicalResult MetalRuntime::mapStorageClassToDescriptorType(
    spirv::StorageClass storageClass, VkDescriptorType &descriptorType) {
  llvm::report_fatal_error ("MetalRuntime::mapStorageClassToDescriptorType");
...
LogicalResult MetalRuntime::mapStorageClassToBufferUsageFlag(
    spirv::StorageClass storageClass, VkBufferUsageFlagBits &bufferUsage) {
  llvm::report_fatal_error ("MetalRuntime::mapStorageClassToBufferUsageFlag");
//   switch (storageClass) {
//   case spirv::StorageClass::StorageBuffer:
...
LogicalResult MetalRuntime::countDeviceMemorySize() {
  llvm::report_fatal_error ("MetalRuntime::countDeviceMemorySize");
...
LogicalResult MetalRuntime::initRuntime() {
	  llvm::report_fatal_error ("MetalRuntime::initRuntime");

//   if (!resourceData.size()) {
//     llvm::errs() << "Metal runtime needs at least one resource";
//     return failure();
//   }
...
LogicalResult MetalRuntime::destroy() {
  llvm::report_fatal_error ("MetalRuntime::destroyu");
	
//   // According to Metal spec:
...
//   // an application is responsible for destroying/freeing any Metal objects
//   // that were created using that device as the first parameter of the
//   // corresponding vkCreate* or vkAllocate* command."
//   RETURN_ON_METAL_ERROR(vkDeviceWaitIdle(device), "vkDeviceWaitIdle");
// 
//   // Free and destroy.
...
LogicalResult MetalRuntime::run() {
  llvm::report_fatal_error ("MetalRuntime::run");
...
//     return failure();
// 
//   RETURN_ON_METAL_ERROR(vkQueueWaitIdle(queue), "vkQueueWaitIdle");
   return success();
}

LogicalResult MetalRuntime::createInstance() {
  llvm::report_fatal_error ("MetalRuntime::createInstance");
//	
//  VkApplicationInfo applicationInfo = {};
//  applicationInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
//  applicationInfo.pNext = nullptr;
//  applicationInfo.pApplicationName = "MLIR Metal runtime";
...
//
//  RETURN_ON_METAL_ERROR(vkCreateInstance(&instanceCreateInfo, 0, &instance),
//                         "vkCreateInstance");
  return success();
}

LogicalResult MetalRuntime::createDevice() {
  llvm::report_fatal_error ("MetalRuntime::createDevice");
//   uint32_t physicalDeviceCount = 0;
//   RETURN_ON_METAL_ERROR(
//       vkEnumeratePhysicalDevices(instance, &physicalDeviceCount, 0),
//       "vkEnumeratePhysicalDevices");
// 
//   llvm::SmallVector<VkPhysicalDevice, 1> physicalDevices(physicalDeviceCount);
//   RETURN_ON_METAL_ERROR(vkEnumeratePhysicalDevices(instance,
//                                                     &physicalDeviceCount,
//                                                     physicalDevices.data()),
//                          "vkEnumeratePhysicalDevices");
// 
//   RETURN_ON_METAL_ERROR(physicalDeviceCount ? VK_SUCCESS : VK_INCOMPLETE,
//                          "physicalDeviceCount");
// 
...
//   }
// 
//   RETURN_ON_METAL_ERROR(memoryTypeIndex == VK_MAX_MEMORY_TYPES ? VK_INCOMPLETE
//                                                                 : VK_SUCCESS,
...
#if 0
LogicalResult
MetalRuntime::getBestComputeQueue(const VkPhysicalDevice &physicalDevice) {
  llvm::report_fatal_error ("MetalRuntime::getBestComputeQueue");
...
LogicalResult MetalRuntime::createMemoryBuffers() {
  llvm::report_fatal_error ("MetalRuntime::createMemoryBuffers");
//  // For each descriptor set.
//  for (const auto &resourceDataMapPair : resourceData) {
//    llvm::SmallVector<MetalDeviceMemoryBuffer, 1> deviceMemoryBuffers;
...
//      MetalDeviceMemoryBuffer memoryBuffer;
...
//      RETURN_ON_METAL_ERROR(vkAllocateMemory(device, &memoryAllocateInfo, 0,
//                                              &memoryBuffer.deviceMemory),
//                             "vkAllocateMemory");
//      void *payload;
//      RETURN_ON_METAL_ERROR(vkMapMemory(device, memoryBuffer.deviceMemory, 0,
//                                         bufferSize, 0,
...
//      RETURN_ON_METAL_ERROR(vkBindBufferMemory(device, memoryBuffer.buffer,
...
LogicalResult MetalRuntime::createShaderModule() {
  llvm::report_fatal_error ("MetalRuntime::createShaderModule");
...
void MetalRuntime::initDescriptorSetLayoutBindingMap() {
  llvm::report_fatal_error ("MetalRuntime::initDescriptorSetLayoutBindingMap");
...
LogicalResult MetalRuntime::createDescriptorSetLayout() {
  llvm::report_fatal_error ("MetalRuntime::createDescriptorSetLayout");
//   for (const auto &deviceMemoryBufferMapPair : deviceMemoryBufferMap) {
//     const auto descriptorSetIndex = deviceMemoryBufferMapPair.first;
//     const auto &deviceMemoryBuffers = deviceMemoryBufferMapPair.second;
...
LogicalResult MetalRuntime::createPipelineLayout() {
  llvm::report_fatal_error ("MetalRuntime::createPipelineLayout");
...
//   RETURN_ON_METAL_ERROR(vkCreatePipelineLayout(device,
...
LogicalResult MetalRuntime::createComputePipeline() {
  llvm::report_fatal_error ("MetalRuntime::createComputePipeline");
...
//   RETURN_ON_METAL_ERROR(vkCreateComputePipelines(device, 0, 1,
...
LogicalResult MetalRuntime::createDescriptorPool() {
  llvm::report_fatal_error ("MetalRuntime::createDescriptorPool");
...
//   RETURN_ON_METAL_ERROR(vkCreateDescriptorPool(device,
...
LogicalResult MetalRuntime::allocateDescriptorSets() {
  llvm::report_fatal_error ("MetalRuntime::allocateDescriptorSets");
...
//   RETURN_ON_METAL_ERROR(vkAllocateDescriptorSets(device,
...
LogicalResult MetalRuntime::setWriteDescriptors() {
  llvm::report_fatal_error ("MetalRuntime::setWriteDescriptors");
...
LogicalResult MetalRuntime::createCommandPool() {
  llvm::report_fatal_error ("MetalRuntime::createCommandPool");
...
LogicalResult MetalRuntime::createComputeCommandBuffer() {
  llvm::report_fatal_error ("MetalRuntime::createComputeCommandBuffer");
...
//  RETURN_ON_METAL_ERROR(vkAllocateCommandBuffers(device,
...
//
//  // Commands end.
//  RETURN_ON_METAL_ERROR(vkEndCommandBuffer(commandBuffer),
//                         "vkEndCommandBuffer");
//
...
LogicalResult MetalRuntime::submitCommandBuffersToQueue() {
  llvm::report_fatal_error ("MetalRuntime::submitCommandBuffersToQueue");
...
//  RETURN_ON_METAL_ERROR(vkQueueSubmit(queue, 1, &submitInfo, 0),
//                         "vkQueueSubmit");
  return success();
}

LogicalResult MetalRuntime::updateHostMemoryBuffers() {
  llvm::report_fatal_error ("MetalRuntime::updateHostMemoryBuffers");
//  // For each descriptor set.
...
//        RETURN_ON_METAL_ERROR(vkMapMemory(device,
