//===- ConvertLaunchFuncToMetalCalls.cpp - MLIR Metal conversion passes -===//
//
...
// Metal runtime calls. The Metal runtime API surface is huge so currently we
// don't expose separate external functions in IR for each of them, instead we
// expose a few external functions to wrapper libraries which manages Metal
// runtime.
//
//===----------------------------------------------------------------------===//
...
static constexpr const char *kRunOnMetal = "runOnMetal";
static constexpr const char *kSPIRVBinary = "SPIRV_BIN";

namespace {

/// A pass to convert gpu.launch_func operation into a sequence of Metal
/// runtime calls.
///
...
/// * runOnMetal          -- runs metal runtime
///
class GpuLaunchFuncToMetalCalssPass
    : public ModulePass<GpuLaunchFuncToMetalCalssPass> {
private:
  ...
  void declareMetalFunctions(Location loc);

  /// Translates the given `launcOp` op to the sequence of Metal runtime calls
...
void GpuLaunchFuncToMetalCalssPass::runOnModule() {
...
void GpuLaunchFuncToMetalCalssPass::declareMetalFunctions(Location loc) {
  ...
  if (!module.lookupSymbol(kRunOnMetal)) {
    builder.create<LLVM::LLVMFuncOp>(
        loc, kRunOnMetal,
        LLVM::LLVMType::getFunctionTy(getVoidType(), {},
                                      /*isVarArg=*/false));
...
Value GpuLaunchFuncToMetalCalssPass::createEntryPointNameConstant(
...
LogicalResult GpuLaunchFuncToMetalCalssPass::createBinaryShader(
...
LogicalResult GpuLaunchFuncToMetalCalssPass::createNumWorkGroups(
...
void GpuLaunchFuncToMetalCalssPass::translateGpuLaunchCalls(
    mlir::gpu::LaunchFuncOp launchOp) {
  ModuleOp module = getModule();
  ...
  if (failed(
          GpuLaunchFuncToMetalCalssPass::createBinaryShader(module, binary)))
  ...
  // Create call to `runOnMetal` runtime function.
  builder.create<LLVM::CallOp>(loc, ArrayRef<Type>{getVoidType()},
                               builder.getSymbolRefAttr(kRunOnMetal),
                               ArrayRef<Value>{});

  // Declare runtime functions.
  declareMetalFunctions(loc);

  launchOp.erase();
}

std::unique_ptr<mlir::OpPassBase<mlir::ModuleOp>>
mlir::createConvertGpuLaunchFuncToMetalCallsPass() {
  return std::make_unique<GpuLaunchFuncToMetalCalssPass>();
}

static PassRegistration<GpuLaunchFuncToMetalCalssPass>
    pass("launch-func-to-metal",
         "Convert gpu.launch_func op to Metal runtime calls");
